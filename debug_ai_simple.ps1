# AI Model "Server Error" Debugging Script - Simple Version
...
$containers = docker ps --format "table {{.Names}}`t{{.Status}}" | Select-String -Pattern "gharsewa"
if ($containers) {
    Write-Host "OK - Docker containers running:" -ForegroundColor Green
    docker ps --format "table {{.Names}}`t{{.Status}}`t{{.Ports}}" | Select-String -Pattern "gharsewa"
} else {
    Write-Host "ERROR - No Gharsewa containers running!" -ForegroundColor Red
    Write-Host "  Fix: cd backend; docker-compose up -d" -ForegroundColor Yellow
    exit 1
}
Write-Host ""
...
if ($ollamaRunning) {
    Write-Host "OK - Ollama container is running" -ForegroundColor Green
} else {
    Write-Host "ERROR - Ollama container is NOT running!" -ForegroundColor Red
    Write-Host "  Fix: cd backend; docker-compose -f docker-compose.ollama.yml up -d" -ForegroundColor Yellow
    exit 1
}
Write-Host ""
...
if ($modelCheck -match "qwen3-vl:2b") {
    Write-Host "OK - Qwen 3.5 VL 2B model is loaded" -ForegroundColor Green
} else {
    Write-Host "ERROR - Qwen 3.5 VL 2B model is NOT loaded!" -ForegroundColor Red
    Write-Host "  Fix: docker exec gharsewa_ollama ollama pull qwen3-vl:2b" -ForegroundColor Yellow
...
try {
    ...
    if ($ollamaResponse) {
        Write-Host "OK - Ollama API is responding on localhost:11434" -ForegroundColor Green
    }
} catch {
    Write-Host "ERROR - Ollama API is NOT responding on localhost:11434" -ForegroundColor Red
...
if ($pingTest -match "2 received") {
    Write-Host "OK - Backend can reach Ollama container (ping successful)" -ForegroundColor Green
} else {
    ...
    Write-Host "  Fix: Check Docker network configuration" -ForegroundColor Yellow
}
Write-Host ""
...
if ($envCheck) {
    Write-Host "OK - Ollama environment variables found:" -ForegroundColor Green
    docker exec gharsewa_app printenv | Select-String -Pattern "OLLAMA"
} else {
    Write-Host "ERROR - Ollama environment variables NOT found!" -ForegroundColor Red
    Write-Host "  Fix: Check backend/.env file" -ForegroundColor Yellow
}
Write-Host ""
...
if ($routeCheck) {
    Write-Host "OK - AI consultation routes found:" -ForegroundColor Green
    docker exec gharsewa_app php artisan route:list 2>&1 | Select-String -Pattern "consultations"
} else {
    Write-Host "ERROR - AI consultation routes NOT found!" -ForegroundColor Red
    Write-Host "  Fix: docker exec gharsewa_app php artisan route:cache" -ForegroundColor Yellow
}
Write-Host ""
...
if ($testGeneration -match "response") {
    Write-Host "OK - Backend can successfully call Ollama API!" -ForegroundColor Green
    Write-Host "  Response preview: $($testGeneration.Substring(0, [Math]::Min(100, $testGeneration.Length)))..." -ForegroundColor Gray
} else {
    Write-Host "ERROR - Backend CANNOT call Ollama API!" -ForegroundColor Red
...
if ($networkCheck -match "gharsewa_app" -and $networkCheck -match "gharsewa_ollama") {
    Write-Host "OK - Both containers are on the same network (gharsewa-network)" -ForegroundColor Green
} else {
    Write-Host "ERROR - Containers may not be on the same network!" -ForegroundColor Red
    Write-Host "  Fix: Check docker-compose.yml network configuration" -ForegroundColor Yellow
}
Write-Host ""
...
Write-Host "If all checks passed (OK), the infrastructure is working correctly." -ForegroundColor Green
...
Write-Host "If any checks failed (ERROR), fix those issues first." -ForegroundColor Red
Write-Host ""
