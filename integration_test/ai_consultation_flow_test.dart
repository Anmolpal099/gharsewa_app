...
void main() {
  ...
  group('AI Visual Assistant Integration Tests', () {
    ...
    testWidgets('Full consultation creation flow - image capture to results',
        (WidgetTester tester) async {
      // Setup mock responses
      final testImage = createTestImageFile();
      ...
      // Note: Actual camera/gallery interaction requires platform integration
      // In integration tests, we verify the UI elements are present
      // and the flow can be triggered
    });

    testWidgets('Annotation workflow - add markers and descriptions',
        (WidgetTester tester) async {
      final testImage = createTestImageFile();
      ...
      // Note: Tapping on canvas to add markers requires gesture simulation
      // which is complex in integration tests. We verify the canvas is present
      // and interactive elements are available.
    });
    ...
    testWidgets('History loading and viewing', (WidgetTester tester) async {
      ...
      // Note: Actual list items depend on the provider implementation
    ...
    testWidgets('Consultation deletion with confirmation',
      ...
      // Note: Actual deletion flow requires tapping on list items
      // and confirming deletion dialog. We verify the screen is functional.
    });
    ...
    testWidgets('Error scenario - API timeout handling',
      ...
      // Note: Actual timeout UI depends on implementation
    });
    ...
    testWidgets('Analysis results screen displays correctly',
        (WidgetTester tester) async {
      final mockConsultation = createMockConsultation();
      ...
      // Verify results screen displays key information
      expect(find.text('AI Diagnosis'), findsOneWidget);

      // Note: Actual content display depends on state management
      // We verify the screen renders without errors
    });
    ...
    test('DefectMarker model validation', () {
      // Test valid marker
      final validMarker = DefectMarkerModel(
        id: 'test-id',
        x: 0.5,
        y: 0.5,
      ...
      final invalidCoords = DefectMarkerModel(
        id: 'test-id',
        x: 1.5, // Invalid: > 1.0
        y: 0.5,
        description: 'Valid description',
      ...
      final invalidDesc = DefectMarkerModel(
        id: 'test-id',
        x: 0.5,
        y: 0.5,
    ...
    test('ProviderRecommendation model properties', () {
      final provider = ProviderRecommendationModel(
        id: 'provider-1',
        name: 'Expert Plumbers',
        rating: 4.8,
        services: ['Plumbing Repair', 'Pipe Installation'],
